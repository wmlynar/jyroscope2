...
public abstract class ProcessTree
    ...
    /**
     * Woj: hack to let non-sudo process kill other processes with sudo. User needs to be added to sudoers.
     */
	public static boolean KILL_WITH_SUDO = false;
    ...
    public abstract class UnixProcess extends OSProcess {
        ...
        public void kill() throws InterruptedException {
            try {
                int pid = getPid();
                if(ProcessTree.KILL_WITH_SUDO) {
                    LOGGER.fine("Killing with sudo pid=" + pid);
                	String[] command = new String[] { "bash", "-c", "sudo kill -9 " + pid };
                	try {
