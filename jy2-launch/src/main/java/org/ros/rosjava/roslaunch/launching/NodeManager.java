	...
	private static void printNamespaceMap(final Map<String, List<NodeTag> > namespaceMap)
	{
		for (String namespace : namespaceMap.keySet())
		{
			...
			for (NodeTag node: nodes)
			{
				...
				// print parameters
				for (ParamTag p : node.getParams()) {
					PrintLog.info("        " + p.getParamName() + "=" + p.getValue());
				}
				PrintLog.info("      remappings:");
				// print parameters
				for (Entry<String, String> r : node.getRemappings().entrySet()) {
					PrintLog.info("        " + r.getKey() + "=" + r.getValue());
				}
			}
		}
	}
...
//		String processName = ProcessMonitor.getProcessName(node.getResolvedName());
		String processName = node.getResolvedName();
		...
		if(command[0].trim().endsWith(".jar")) {
			String[] args = new String[command.length - 1];
			for (int i = 0; i < args.length; i++) {
				args[i] = command[i + 1];
			}
			// ThreadGroup tg = JarUtils.runJar(command[0], args);
			ThreadGroup tg = null;
			JarUtilsCollected.addJar(command[0], args);
